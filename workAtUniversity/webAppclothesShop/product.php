...
<?
	...
	if(isset($_GET["by"]))
		$result = mysql_query("SELECT * FROM store WHERE product_type = '".$_GET["by"]."' ORDER BY no DESC");
	elseif(isset($_GET["search"]))
		$result = mysql_query("SELECT * FROM store WHERE product_name LIKE '%".$_GET["search"]."%' ORDER BY no DESC");
	else
		$result = mysql_query("SELECT * FROM store ORDER BY no DESC");
?>
...
<br/>
	<center>
<form name="search" method="get" action="product.php">
    	<? if(isset($_GET["by"])){echo $_GET["by"];}elseif(isset($_GET["search"])){echo "ผลการค้นหา";}else{echo "สินค้าทั้งหมด";}?>
        <table>
            <? if(mysql_num_rows($result)!=0)
			{
				...
				<? for($i=0;$row = mysql_fetch_array($result);$i++)
				{
					if($i%3 != 2)
					{?>
                    	<td><center><a href="details.php?id=<?=$row["product_id"];?>"><img src="img/resize_<?=$row["product_img"];?>"><br /><? echo $row["product_name"];?></a><br /><? echo $row["product_cost"];?> บาท</center></td>
				 <? }
				 	else
					{?>
                    	<td><center><a href="details.php?id=<?=$row["product_id"];?>"><img src="img/resize_<?=$row["product_img"];?>"><br /><? echo $row["product_name"];?></a><br /><? echo $row["product_cost"];?> บาท</center></td></tr><tr>
				 <? }
            	}?>
				</tr>
			<? }
			else{?><tr><td>ไม่พบสินค้าที่ค้นหา</td></tr><? }?>
            <? mysql_close($con);?>
        </table>
    </form>
	</center>
