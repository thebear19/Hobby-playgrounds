...
<title>Thank You For Ordered</title>
...
<script type="text/JavaScript">
	...
	{
		setTimeout("window.location='home.php';",timeoutPeriod);
	}
</script>
...
<body style="background-image: url(images/BG001.jpg);" bgproperties="fixed" onLoad="JavaScript:timedout(4000);">
<center>
<div id="container">
<div id="header"></div>
...
<br/>
	...
	<?
		if($_GET["order"])
		{
			...
			$result = mysql_query("SELECT * FROM orders WHERE order_id ='".$_GET["id"]."'");
			while($row = mysql_fetch_array($result))
			{
				$re = mysql_query("SELECT * FROM store WHERE product_id ='".$row["product_id"]."'");
				$row2 = mysql_fetch_array($re);
				$total = $row2["product_amount"]-$row["amount"];
				$sql = "UPDATE store SET product_amount = '$total' WHERE product_id = '".$row["product_id"]."'";
				mysql_query($sql,$con);
			}
			mysql_query("UPDATE orders SET order_status = 'submit' WHERE order_id = '".$_GET["id"]."'");
			?>
        	<center>บันทึกการทำรายการเรียบร้อยแล้ว<br />
			<a href="home.php">คลิกที่นี่เพื่อกลับสู่หน้าหลัก</a></center>
		<? }
		else
		{
