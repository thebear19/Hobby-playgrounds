...
<script src="Script/jsl.js" type="text/javascript"></script>
...
<?
	if($_SESSION["logined"])
	{
		$user = $date = $idbuy = $pay = false;
		if(isset($_POST["submit"]))
		{
			if(empty($_POST["date"])){$date = true;}
			if(empty($_POST["idbuy"])){$idbuy = true;}
			if(empty($_POST["pay"])){$pay = true;}
			if(!($date || $idbuy || $pay))
			{	
				$id = mysql_query("SELECT MAX(payment_id) FROM payments");
				$id = mysql_fetch_array($id);
				$id = $id["MAX(payment_id)"];
				$id++;
				$time = $_POST["date"]." ".$_POST["hour"].":".$_POST["min"].":".date("s");
				$sql = "INSERT INTO payments VALUES('$id','".$_SESSION["username"]."','$time','".$_POST["idbuy"]."','".$_POST["pay"]."','".$_POST["bank"]."','no')";
				mysql_query($sql,$con);
				$_SESSION["payed"] = true;
...
<br/>
	<form name="payment" method="post" action="payment.php">
    	<img border="0" src = "images/text-Pay.png"/><br /><br />
    	<table>
        	<tr>
            	<td><img border="0" src = "images/text-PayUser.png"/></td>
    			<td><? echo $_SESSION["username"];?></td>
            </tr>
            
            <tr>
            	<td><img border="0" src = "images/text-PayDate.png"/> </td>
    			<td><input type="text" name="date" id="date" value="<? if(!$date && isset($_POST["submit"])){echo $_POST["date"];}?>" /><? if($date){echo "กรุณากรอกวันที่ทำการโอน";}?></td>
            </tr>
            
            <tr>
            	<td><img border="0" src = "images/text-PayTime.png"/></td>
    			<td><select size="1" name="hour">
					<? for($i=0;$i<24;$i++){echo "<option value="."$i".">$i</option>";}?>
                	</select><img border="0" src = "images/text-PaySemi.png"/><select size="1" name="min">
					<? for($i=0;$i<60;$i++){echo "<option value="."$i".">$i</option>";}?>
                	</select></td>
            </tr>
            
            <tr>
            	<td><img border="0" src = "images/text-PayCode.png"/> </td>
    			<td><input type="text" name="idbuy" value="" /><? if($idbuy){echo "กรุณากรอกรหัสใบสั่งซื้อ";}?></td>
            </tr>
            
            <tr>
            	<td><img border="0" src = "images/text-PayMoney.png"/> </td>
    			<td><input type="text" name="pay" value="0.00" /><img border="0" src = "images/text-PayBath.png"/> <? if($pay){echo "กรุณากรอกจำนวนเงิน";}?></td>
            </tr>
            
            <tr>
            	<td><img border="0" src = "images/text-PayBank.png"/></td>
    			<td><input type="radio" name="bank" value="kasikorn" checked="checked" /><img src="images/Kasikorn.jpg" title="" alt="กสิกร" width="20" height="20"><img border="0" src = "images/text-PayBank1.png"/><br /><br />
                	<input type="radio" name="bank" value="scb" /><img src="images/SCBB.png" title="" alt="ไทยพาณิชย์" width="20" height="20"><img src = "images/text-PayBank3.png"/><br /><br /></td>
            </tr>
            
            <tr>
    			<td colspan="2"><center><input type="image" name="submit" value="Submit" src = "images/buttom-Submit.png"/><input type="image" name="cancel" value="Cancel" src = "images/buttom-Cancel.png"/></center></td>
            </tr>
