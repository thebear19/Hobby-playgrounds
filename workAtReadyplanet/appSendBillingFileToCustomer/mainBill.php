...
//ถ้าจะนำไปใช้กับอย่างอื่นแนะนำโหลดตัวเต็ม แล้วมาปรับแต่งเอา วิธีใช้เบื้องต้น นำไฟล์นี้ไปวางไว้ที่ part ที่ต้องการ Folder นั้น จะเป็น root
...
.quick {display:inline-block; float:left; margin-top: 35px; margin-right: 15px;}
</style>
...
$(function(){
	...
	$('#file_drop_target').bind('dragover',function(){
		...
		$.each(files,function(k,file) {
                    if(window.location.hash.substr(1) == 'outbox'){
			uploadFile(file);
                    }
                    else{
                        alert("This location can't uploads file.");
                        return false;
                    }
		});
		$(this).removeClass('drag_over');
	...
	$('input[type=file]').change(function(e) {
        ...
        $('button[name="quickmail"]').click(function (){
            if(confirm("All files in outbox are being sent to customers, Are you sure to continue this process?")){
                $.ajax({
                    url: "system/sendBilling.php",
                    type: "POST",
                    beforeSend: function () {
                        $('button[name="quickmail"]').html("Loading...");
                    },
                    success: function() {
                        alert("Send complete");
                    },
                    ...
                    complete: function () {
                        $('button[name="quickmail"]').html("Quick sendmail");
                    }
                });
            }
        });
...
<div id="top">
	...
	<div id="file_drop_target">
        ...
        <button name="quickmail" class="quick">Quick sendmail</button>
